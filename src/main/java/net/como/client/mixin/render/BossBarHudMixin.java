...
public class BossBarHudMixin {
    @Inject(at = @At("HEAD"), method="render(Lnet/minecraft/client/gui/DrawContext;)V", cancellable = true)
    public void onRender(DrawContext context, CallbackInfo ci) {
        ComoClient.getInstance().emitter.triggerEvent(new BossBarHudRenderEvent(context, ci));
    }
    ...
    public void onShouldDarkenSky(CallbackInfoReturnable<Boolean> cir) {
        ComoClient.getInstance().emitter.triggerEvent(new BossBarHudSkyEffectsEvent(cir));
    }
